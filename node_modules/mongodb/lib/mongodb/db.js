  ...
  return function(results) {
    ...
    if(Array.isArray(results)) {
      for(var i = 0; i < results.length; i++) {
        index = results[i].name.indexOf(matching);
      ...
      index = results.name.indexOf(matching);
      if(index == 0) {
 ...
 * NOTE: when using {name: /rexexp/} for MongoDB 2.X, match is done on names with
 * database name prefix: 'dbName.collection'
...
Db.prototype.listCollections = function(filter, options) {
  ...
  if(connection && connection.serverCapabilities.hasListCollectionsCommand) {
    ...
    // Build the command
    command = { listCollections : true, filter: filter, cursor: cursor };
  ...
  if(connection && !connection.serverCapabilities.hasListCollectionsCommand) {
    ...
    if(typeof filter.name == 'string' && 0 != filter.name.indexOf(this.databaseName + '.')) {
      filter = utils.shallowObjectCopy(filter);
      filter.name = f('%s.%s', this.databaseName, filter.name);
    }
  }

  // Filter out indexes, e.g.:
  // { name: 'i31.$_id_' },  { name: 'i31.$id_1' }
  // for collection:
  // { name: 'i31' }
  command = {name:/^[^$]*$/}
  // Rewrite the filter to use $and to filter out indexes
  if(filter.name) {
    filter = {$and: [{name: filter.name}, command]};
  } else {
    filter = command;
  }

  // Return options
  options = {transforms: transformCollections(this.databaseName)}
  // Get the cursor
  cursor = this.collection(DbCommand.SYSTEM_NAMESPACE_COLLECTION).find(filter, options);
