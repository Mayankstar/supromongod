...
var cmd, cp, fs
    ...
    cmd = {// check and apply defaults
        bin: cfg.bin ? cfg.bin :'/usr/local/bin/mongod',
        arg:(
            cfg.cmd_launch ||
...
function mongodb_connect(app_api, config){
    ...
    * */
    ...
    if(!cfg.url) cfg.url = 'mongodb://127.0.0.1:27727/'
    if(!cfg.db_name) cfg.db_name = 'supro_GLOB'

    if(!cfg.extjs) cfg.extjs = { mongodb_port: 0 }
    if(!cfg.extjs.mongodb_port) cfg.extjs.mongodb_port = 27727// for link
...
function end_with_mongodb(next){
    ...
    function get_admin(aerr, a){
        ...
        //"! MongoDB shutdown command error: [Error: connection closed]"
            //err && log('! MongoDB shutdown command error:', err)
